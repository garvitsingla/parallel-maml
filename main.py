...
mission_encoder_output_dim = 32
sampler_lang.mission_encoder = MissionEncoder(len(sampler_lang.vectorizer.get_feature_names_out()), 32, 64, mission_encoder_output_dim).to(device)
sampler_lang.mission_encoder.load_state_dict(ckpt_2["mission_encoder"])
sampler_lang.mission_encoder.eval()
...
for i in range(N_MISSIONS):
    ...
    for ep in range(N_EPISODES):
        env.reset_task(mission)
        steps = evaluate_policy(env, policy_unadapted_lang, preprocess_obs= sampler_lang.preprocess_obs, params=theta_prime_unadapted)
...
print(f"ablation: {np.mean(results_lang_unadapted):.2f} ± {np.std(results_lang_unadapted):.2f}")
